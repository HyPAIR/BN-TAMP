...
;                        ;(not (UnsafeTraj ?t))
;                   )
;     :effect (and (AtPose ?o ?p) (HandEmpty) (CanMove) ;(Clear ?b)
;                  (not (AtGrasp ?o ?g)))
;   )

;   ; (:action unstack
;   ; :parameters (?b ?p ?g ?q ?b-below)
;   ; :precondition (and (AtConf ?q) (OnNew ?b ?b-below) (Clear ?b)
;   ;                    (AtPose ?b ?p) (HandEmpty))
;   ; :effect (and (AtGrasp ?b ?g) (not (OnNew ?b ?b-below)) (Clear ?b-below)
;   ;               (not (HandEmpty))))

;   ; (:action unstack
;   ;   :parameters (?b ?p ?g ?q ?b-below)
;   ;   :precondition (and  (Kin ?b ?q ?p ?g)
;   ;                      (AtConf ?q) (On ?b ?b-below) (Clear ?b)
;   ;                      (AtPose ?b ?p) (HandEmpty))
;   ;   :effect (and (AtGrasp ?b ?g) (CanMove)
;   ;                (not (Clear ?b)) 
;   ;                (not (AtPose ?b ?p)) (not (On ?b ?b-below))
;   ;                (Clear ?b-below) (not (HandEmpty)))
...
; ;   (:derived (Supported ?o ?p ?r)
; ;   (and (AtPose ?o ?p) (On ?o ?r))
; ; )
